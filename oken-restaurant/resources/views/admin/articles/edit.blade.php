    ...
    <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" required
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black sm:text-sm">{{ old('description', $article->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <div class="mt-1 relative rounded-md shadow-sm">
                ...
                <input type="number" name="price" id="price" step="0.01" value="{{ old('price', $article->price) }}" required
        ...
        <div>
            ...
            <select name="category" id="category" required
                ...
                <option value="main" {{ old('category', $article->category) == 'main' ? 'selected' : '' }}>Main</option>
                <option value="dessert" {{ old('category', $article->category) == 'dessert' ? 'selected' : '' }}>Dessert</option>
                <option value="starter" {{ old('category', $article->category) == 'starter' ? 'selected' : '' }}>Starter</option>
            </select>
            @error('category')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <select name="restaurant_id" id="restaurant_id" required
                ...
                @foreach($restaurants as $restaurant)
                    <option value="{{ $restaurant->id }}" {{ old('restaurant_id', $article->restaurant_id) == $restaurant->id ? 'selected' : '' }}>
                        {{ $restaurant->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            @if($article->image)
                <div class="mt-2">
                    <img src="{{ Storage::url($article->image) }}" alt="{{ $article->name }}" class="h-32 w-32 object-cover rounded-lg">
                </div>
            @endif
